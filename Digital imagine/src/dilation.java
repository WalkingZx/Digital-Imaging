...
public class dilation {
	 ...
	 * @return the Mat after dilating by using opencv
	 */
	...
	/**@Description gray processing image, traversing the matrix generated by the image to find the maximum value of the 5 * 5 matrix where each value is located
	 * 
	 * @param m the Mat of the original picture
	 * @return the Mat after dilating by using myself
	 */
			...
			for(int j=0; j<height; j++) {
				...
				for(int x=i-2; x<i+3; x++) {
					for(int y=j-2; y<j+3; y++) {
						...
						if(x>=0 && y>=0 && x<width && y<height && m.get(x, y)[0] > max[0]) {
							max = m.get(x, y);}
